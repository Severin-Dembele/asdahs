...
function AdminImpactSocietalQuestions() {
  ...
  const reloadData = async () => {
    try {
      const adminRes = await getData(ENDPOINT.questions);
  ...
  const deleteDta = async (id) => {
    ...
    try {
      const response = await deleteDta(`${ENDPOINT.questions}/${id}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      let endpoint = ENDPOINT.questions;
      let response;

      if (formData?.id) {
        endpoint = `${ENDPOINT.questions}/${formData?.id}`;
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between text-center">
        <p className="mt-3 mb-6 text-2xl font-bold">List of questions</p>
        <button
                ...
                <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                  ...
                  <div className="flex justify-center">
                    <button
                      onClick={() => {
                        navigation(
                          `/africanhealthstudy/panel-administration/details-question/${item?.id}`
                        );
                      }}
...
export default AdminImpactSocietalQuestions;

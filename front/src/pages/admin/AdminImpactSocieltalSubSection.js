...
function AdminImpactSocieltalSubSection() {
  ...
  const reloadData = async () => {
    try {
      const adminRes = await getData(ENDPOINT.subsections);
  ...
  const deleteDta = async (id) => {
    ...
    try {
      const response = await deleteDta(`${ENDPOINT.subsections}/${id}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      let endpoint = ENDPOINT.subsections;
      let response;

      if (formData?.id) {
        endpoint = `${ENDPOINT.subsections}/${formData?.id}`;
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between text-center">
        <p className="mt-3 mb-6 text-2xl font-bold">List of sub sections</p>
        <button
              ...
              >
                ...
                <div className="flex flex-wrap m-10 space-x-2 space-y-2">
                  ...
                  >
                    ...
                    <span>Add questions to subsection</span>
                  </button>
                </div>
              </div>
            ))}
      ...
      <Modal show={isModal} onClose={() => setIsModal(false)}>
        <Modal.Header>Sub section</Modal.Header>
        <Modal.Body>
          <div className="flex flex-col gap-1">
            <div>
              <div className="block mb-2">
                <Label htmlFor="name" value="Title of the subsection" />
              </div>
              <TextInput
                id="title"
                sizing="md"
            ...
            <div>
              <div className="block mb-2">
                <Label
                  htmlFor="message"
                  value="Description of the subsection"
                />
              </div>
              <Textarea
                id="description"
...
export default AdminImpactSocieltalSubSection;

...
function AdminSettingConference() {
    ...
    const deleteDta = async (id) => {
        ...
        try {
            const response = await deleteDta(`${ENDPOINT.conferences}/${id}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            let endpoint = ENDPOINT.unions;
            let response;

            if (formData?.id) {
                endpoint = `${ENDPOINT.unions}/${formData?.unionId}/${ENDPOINT.conferences}/${formData?.id}`;
                response = await putDataWithNoToken(endpoint, formData, false);
            } else {
                response = await postDataWithNoToken(`${ENDPOINT.unions}/${formData?.unionId}/${ENDPOINT.conferences}`, formData, false);
            }
    ...
    return (
        <div>
            <div className="flex flex-wrap items-center justify-between text-center">
                <p className="mt-3 mb-6 text-2xl font-bold">List of Conference</p>
            </div>
            ...
            <Modal show={isModal} onClose={() => setIsModal(false)}>
                <Modal.Header>Conference</Modal.Header>
                <Modal.Body>
                    <div className="flex flex-col gap-1">
                        ...
                        <div>
                            <div className="block mb-2">
                                <Label htmlFor="unionId" value="Division" />
                            </div>
                            <Select
                                id="unionId"
                                sizing="md"
...
export default AdminSettingConference;

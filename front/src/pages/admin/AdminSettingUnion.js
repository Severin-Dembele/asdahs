...
function AdminSettingUnion() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (formData?.id) {
                endpoint = `${ENDPOINT.divisions}/${formData?.divisionId}/${ENDPOINT.unions}/${formData?.id}`;
                response = await putDataWithNoToken(endpoint, formData, false);
            } else {
                response = await postDataWithNoToken(`${ENDPOINT.divisions}/${formData?.divisionId}/${ENDPOINT.unions}`, formData, false);
            }
    ...
    return (
        <div>
            <div className="flex flex-wrap items-center justify-between text-center">
                <p className="mt-3 mb-6 text-2xl font-bold">List of Union</p>
            </div>
            ...
            <Modal show={isModal} onClose={() => setIsModal(false)}>
                <Modal.Header>Union</Modal.Header>
                <Modal.Body>
                    <div className="flex flex-col gap-1">
                        <div>
                            <div className="block mb-2">
                                <Label htmlFor="name" value="Union shortname." />
                            </div>
                            <TextInput
                                id="shortname"
                                sizing="md"
                        ...
                        <div>
                            <div className="block mb-2">
                                <Label htmlFor="divisionId" value="Division" />
                            </div>
                            <Select
                                id="divisionId"
                                sizing="md"

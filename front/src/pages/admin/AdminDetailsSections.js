...
function AdminDetailsSections() {
  ...
  const reloadData = async () => {
    try {
      const adminRes = await getData(`${ENDPOINT.sections}/${id}/sub-sections`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      let endpoint = `${ENDPOINT.sections}/${id}/sub-sections`;
      let response;

      if (formData?.id) {
        endpoint = `${ENDPOINT.sections}/${id}/sub-sections`;
          ...
          <div className="flex flex-col gap-1">
            <div>
              <div className="block mb-2">
                <Label htmlFor="name" value="Title of the sub section" />
              </div>
              <TextInput
                id="title"
                sizing="md"
...
export default AdminDetailsSections;
